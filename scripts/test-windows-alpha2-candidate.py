#!/usr/bin/env python3
"""Effect-free tests for the isolated Windows Alpha 2 candidate assembler."""
...
def fixture(root: Path, version: str = MODULE.VERSION) -> Path:
    ...
    (artifacts / MODULE.PORTABLE_ARCHIVE).write_bytes(b"alpha2-candidate")
    for name in MODULE.REQUIRED_EVIDENCE:
        (artifacts / name).write_text("evidence\n", encoding="utf-8")
    provenance = {
        "application": {"name": "Haven 42", "version": version},
        "environment": {"operatingSystem": "windows", "architecture": "amd64"},
...
def main() -> int:
    checks = 0
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-candidate-") as temporary:
        root = Path(temporary)
        ...
        refused(
            ...
            "candidate-archive-integrity-failed",
        )
        checks += 1

    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-limitations-") as temporary:
        root = Path(temporary)
        portable = fixture(root)
        output = root / "candidate"
        with (
    ...
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-limitations-tamper-") as temporary:
        root = Path(temporary)
        portable = fixture(root)
        output = root / "candidate"
        with (
    ...
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-wrong-version-") as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-wrong-host-") as temporary:
        root = Path(temporary)
        portable = fixture(root)
    ...
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-dirty-source-") as temporary:
        root = Path(temporary)
        portable = fixture(root)
        rewrite_provenance(
            portable,
    ...
    with tempfile.TemporaryDirectory(prefix="haven42-alpha2-manifest-source-") as temporary:
        root = Path(temporary)
        portable = fixture(root)
        output = root / "candidate"
        with (
    ...
    alpha1 = (ROOT / "scripts/build-windows-alpha-candidate.py").read_text(encoding="utf-8")
    assert 'VERSION = "0.4.0-alpha.1"' in alpha1
    assert "haven42-{VERSION}-windows-x64-unsigned.zip" in alpha1
    checks += 2
    print(f"Windows Alpha 2 candidate isolation passed: {checks} checks.")
    return 0
