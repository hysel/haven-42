"""Emit allowlisted RTX 4060 evidence as JSON; never copy raw log text."""
import argparse
import hashlib
import json
import math
...
def summarize(directory, platform, hwinfo=None):
    ...
    driver = {'windows': '616.64', 'ubuntu': '610.57.04'}[platform]
    require(state['profile'] == dict(gpu='NVIDIA GeForce RTX 4060', memoryMiB=8188,
        ...
        for phase in ('core', 'soak'):
            ...
            for cell in item[phase]:
                ...
                require(e['modelId'] == mid and e['manifestDigest'] == digest
                        and e['systemMemoryGiB'] == system_memory and e['usableGpuMemoryGiB'] == 7.99609375
    ...
    for field in ('temperatureC', 'powerW'):
        ...
        require(bool(values) or field == 'powerW', 'temperature-unavailable')
        require(all(isinstance(v, (int, float)) and math.isfinite(v) and v >= 0 for v in values), 'invalid-telemetry')
        telemetry[field] = dict(samples=len(values), unavailable=len(rows)-len(values),
                                minimum=min(values) if values else None, maximum=max(values) if values else None,
                                arithmeticMean=round(mean(values), 3) if values else None)
    sources = {name: provenance(directory/name) for name in ('status.json', 'telemetry.jsonl')}
    if hwinfo:
        sources['rawHwinfo'] = provenance(hwinfo)
    return dict(schemaVersion=1, kind='haven42-rtx4060-bounded-inference-evidence',
                observedDate='2026-09-08' if platform == 'windows' else '2026-09-09', platform=platform, operatingSystem=expected_os,
                accelerator='NVIDIA GeForce RTX 4060 8 GB', driverVersion=driver,
