#!/usr/bin/env python3
"""Validate the sanitized Windows GTX 1650 Super qualification result."""
...
RESULT_PATH = ROOT / "config/alpha-2-nvidia-gtx1650-super-windows-qualification-result.json"
...
def main() -> None:
    ...
    assert result["kind"] == "haven42-alpha2-nvidia-gtx1650-super-windows-qualification-result"
    ...
    assert result["environment"]["driverVersion"] == "610.88"
    ...
    power = result["power"]
    assert power["telemetrySampleCount"] == 6437
    ...
    assert "4 GB Windows hardware-fit boundary" in evidence
    assert "retained outside the repository" in " ".join(evidence.split())

    assert PRIVATE_NETWORK.search(result_text) is None
    assert PRIVATE_NETWORK.search(evidence) is None
    ...
    assert result["automaticSelectionEvidenceAllowed"] is True
    assert result["automaticSupportChangeAllowed"] is False

    print("alpha2 Windows GTX 1650 Super qualification result checks passed")
