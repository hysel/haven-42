...
Invoke-PackTest "local web text tools are loopback-only and unload models" {
    ...
    Assert-True -Condition ($externalLinks.Count -eq 4 -and (($observedExternalLinks -join "`n") -eq (($allowedExternalLinks | Sort-Object) -join "`n"))) -Message "Static remote navigation must be limited to the fixed issue form, evidence wiki, and official Ollama release and macOS download pages."
