...
Invoke-PackTest "test tiers are timed and exact-tree receipt gated" {
    ...
    Assert-True -Condition ($pushHook -match "Exact content-tree full-test receipt found" -and $pushHook -match "HEAD\^\{tree\}" -and $pushHook -match "schema=3" -and $pushHook -match "ensure-test-python3\.shared\.sh" -and $pushHook -match "ensure_test_python3") -Message "Pre-push should use validated Python 3 and require a schema-v3 exact content-tree receipt."
    $localEnvironmentClear = $pushHook.IndexOf("git rev-parse --local-env-vars", [System.StringComparison]::Ordinal)
    $privacyValidation = $pushHook.IndexOf("verify-public-repository-privacy.py", [System.StringComparison]::Ordinal)
    Assert-True -Condition ($localEnvironmentClear -ge 0 -and $privacyValidation -gt $localEnvironmentClear) -Message "Pre-push should clear inherited repository-local Git variables before tests create disposable repositories."
...
Invoke-PackTest "core update policy fails closed before cryptographic admission" {
    ...
    $trustContractPath = Join-Path $repoRoot "config/core-update-trust-handoff-contract.json"
    $trustFixturePath = Join-Path $repoRoot "examples/fixtures/core-update-trust-receipt.json"
    $trustScriptPath = Join-Path $repoRoot "scripts/core-update-trust-handoff.py"
    $transitionContractPath = Join-Path $repoRoot "config/core-update-verifier-transition-contract.json"
    $transitionFixturePath = Join-Path $repoRoot "examples/fixtures/core-update-verifier-transition.json"
    $transitionScriptPath = Join-Path $repoRoot "scripts/core-update-verifier-transition.py"
    ...
    foreach ($path in @($trustContractPath, $trustFixturePath, $trustScriptPath)) {
        Assert-True -Condition (Test-Path -LiteralPath $path -PathType Leaf) -Message "Update trust-handoff foundation should exist: $path"
    }
    $trustContract = Get-Content -LiteralPath $trustContractPath -Raw | ConvertFrom-Json
    Assert-True -Condition (-not $trustContract.runtimeAdmitted -and $trustContract.implementationStatus -eq "structural-admission-only" -and -not $trustContract.verification.scenarioValuesAreAuthoritativeEvidence) -Message "Verifier receipts must remain structural, untrusted, and runtime-unadmitted."
    $trustOutput = @(& $python.Source $trustScriptPath --self-test 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "Core update trust-handoff hostile self-test should pass."
    Assert-True -Condition (($trustOutput -join "`n") -match "passed: 30 cases") -Message "Trust handoff should execute every identity, digest, expiry, replay, raw-value, and false-claim case."
    foreach ($path in @($transitionContractPath, $transitionFixturePath, $transitionScriptPath)) {
        Assert-True -Condition (Test-Path -LiteralPath $path -PathType Leaf) -Message "Verifier transition foundation should exist: $path"
    }
    $transitionContract = Get-Content -LiteralPath $transitionContractPath -Raw | ConvertFrom-Json
    Assert-True -Condition (-not $transitionContract.runtimeAdmitted -and $transitionContract.implementationStatus -eq "structural-transition-only" -and -not $transitionContract.authorization.scenarioClaimsAreAuthoritativeEvidence) -Message "Verifier transitions must remain structural, untrusted, and runtime-unadmitted."
    $transitionOutput = @(& $python.Source $transitionScriptPath --self-test 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "Core update verifier-transition hostile self-test should pass."
    Assert-True -Condition (($transitionOutput -join "`n") -match "passed: 33 cases") -Message "Verifier transitions should cover registry continuity, root rotation, expiry, authorization claims, replay, and raw-value rejection."
}
...
Invoke-PackTest "local web text tools are loopback-only and unload models" {
    ...
    $lexicalContractPath = Join-Path $repoRoot "config/lexical-retrieval-contract.json"
    $researchContractPath = Join-Path $repoRoot "config/web-research-contract.json"
    ...
    $researchContract = Get-Content -LiteralPath $researchContractPath -Raw | ConvertFrom-Json
    ...
    Assert-Equal -Actual (($policy.browser.fixedExternalNavigationUrls) -join ",") -Expected "https://github.com/hysel/haven-42/issues/new?template=alpha-bug-report.yml,https://github.com/ollama/ollama/releases,https://ollama.com/download/windows,https://ollama.com/download/linux,https://ollama.com/download/mac" -Message "External navigation must remain fixed to the problem form and official Ollama release and download instructions."
    ...
    Assert-True -Condition (-not $policy.text.automaticUnknownModelSelectionAllowed -and $policy.text.missingModelDownloadsAllowed -and $policy.text.modelDownloads -eq "explicit-user-approval-only" -and $policy.text.recommendationAuthority -eq "server-owned-static-catalog") -Message "Automatic model choice must stay engine-owned and evidence-ranked while manually reviewed downloads remain available."
    Assert-True -Condition ($policy.modelDiscovery.explicitOnlineConsentRequired -and -not $policy.modelDiscovery.redirectsAllowed -and -not $policy.modelDiscovery.automaticDownloadsAllowed -and $policy.modelDiscovery.pullApiAllowed -and $policy.modelDiscovery.explicitInstallApprovalRequired -and $policy.modelDiscovery.installCandidateMustComeFromCurrentSessionSearch -and $policy.modelDiscovery.providerCatalogVerificationRequiredAfterPull -and $policy.modelDiscovery.automaticSelectionAfterInstallAllowed -and $policy.modelDiscovery.evidenceUse -eq "recommendation-and-labeling-only" -and $policy.modelDiscovery.unknownHardwareFit -eq "allow-after-user-approval" -and $policy.modelDiscovery.knownIncompatibleHardwareFit -eq "block-before-download" -and -not $policy.modelDiscovery.commandExecutionAllowed) -Message "Public model discovery must stay explicit and fixed-origin; pulls require one-time approval and only known hardware incompatibility blocks download."
    ...
    Assert-True -Condition ($externalLinks.Count -eq 3 -and (($observedExternalLinks -join "`n") -eq (($allowedExternalLinks | Sort-Object) -join "`n"))) -Message "Static remote navigation must be limited to the fixed issue form and official Ollama release and macOS download pages."
    Assert-True -Condition ($assets -match '"https://ollama\.com/download/linux"' -and $assets -match '"https://ollama\.com/download/windows"' -and $assets -match '"https://ollama\.com/download/mac"') -Message "Manual runtime guidance must choose only the official Ollama download for the detected platform."
    ...
    $pdfArtifactOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-parser-artifact-lock.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF parser artifact-lock tests should pass."
    Assert-True -Condition (($pdfArtifactOutput -join "`n") -match "33 fail-closed checks") -Message "The exact PDF candidate artifact must remain verified but uninstalled and unadmitted."
    $pdfArtifactHostileOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-parser-artifact-hostile.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF parser artifact hostile tests should pass."
    Assert-True -Condition (($pdfArtifactHostileOutput -join "`n") -match "10 cases") -Message "The artifact verifier must reject hostile wheel names, native binaries, expansion, entry counts, renames, and digest tampering."
    $pdfFixtureOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-parser-review-fixtures.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF parser review fixture tests should pass."
    Assert-True -Condition (($pdfFixtureOutput -join "`n") -match "78 checks and created 14 inert files") -Message "The deterministic PDF corpus must cover the required hostile categories without importing a parser or granting runtime authority."
    $pdfWorkerContractOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-worker-prototype-contract.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "Restricted PDF worker prototype contract tests should pass."
    Assert-True -Condition (($pdfWorkerContractOutput -join "`n") -match "64 fail-closed checks") -Message "The review-only PDF worker must retain exact artifact identity, bounded transport, platform containment requirements, and zero runtime or package authority."
    $pdfWorkerBoundaryOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-worker-review-boundary.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF worker contract parity and package-exclusion tests should pass."
    Assert-True -Condition (($pdfWorkerBoundaryOutput -join "`n") -match "40 contract-parity and exclusion checks") -Message "The review-only worker must remain aligned to its limits and excluded from runtime dependencies and packages."
    $pdfNativeFoundationOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-native-validation-foundation.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF native validation planning tests should pass."
    Assert-True -Condition (($pdfNativeFoundationOutput -join "`n") -match "33 offline security and parity checks") -Message "Native PDF planning must preserve bounded execution, sanitized evidence, false package parity, and unrun Linux/macOS states."
    ...
    $pdfResearchOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-pdf-corpus-research-foundation.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "PDF corpus metadata research tests should pass."
    Assert-True -Condition (($pdfResearchOutput -join "`n") -match "19 non-acquisition checks") -Message "PDF corpus research must select and download no external artifact."
...
Invoke-PackTest "task composition and repository privacy foundations fail closed" {
    ...
    $admissionOutput = @(& $python.Source (Join-Path $repoRoot "scripts/test-task-execution-admission.py") 2>&1)
    Assert-Equal -Actual $LASTEXITCODE -Expected 0 -Message "Task execution-admission hostile tests should pass."
    Assert-True -Condition (($admissionOutput -join "`n") -match "49 cases") -Message "Execution admission should cover effect scope, typed intermediates, approvals, retries, cancellation, and recovery."
    ...
    Assert-True -Condition (-not $admission.runtimeAdmitted -and -not $admission.approvalReceipt.opaqueSecretAcceptedBySimulation -and -not $admission.approvalReceipt.rendererMayIssue -and -not $admission.result.approvalAcceptedForExecution -and -not $admission.result.executionAllowed) -Message "Execution-admission simulation must issue no token and grant no execution authority."
    $journal = Get-Content -Raw -LiteralPath (Join-Path $repoRoot "config/task-effect-journal-contract.json") | ConvertFrom-Json
    Assert-True -Condition (-not $journal.runtimeAdmitted -and -not $journal.record.scenarioClaimsAreAuthoritativeEvidence -and -not $journal.result.journalPersisted -and -not $journal.result.effectCompletionProven -and -not $journal.result.executionAllowed) -Message "Effect-journal simulation must persist nothing, prove no effect, and grant no execution authority."
}
...
Invoke-PackTest "system readiness and setup planning remain effect free" {
    ...
    $paths = @(
        ...
        "scripts/alpha2-linux-native-profile-gates.py",
        "scripts/test-alpha2-linux-native-profile-gates.py",
    ...
    foreach ($alphaTest in @(
        ...
        "scripts/test-alpha2-native-package-validation.py",
        "scripts/test-alpha2-promotion-readiness.py",
        "scripts/test-alpha2-hosted-candidate-result.py",
        ...
        "scripts/test-linux-alpha2-candidate.py",
        "scripts/test-verify-alpha2-candidate-pair.py",
