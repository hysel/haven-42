...
Invoke-PackTest "local web text tools are loopback-only and unload models" {
    ...
    Assert-True -Condition ($imageLicenseOutput.Output -match "24 fail-closed checks") -Message "Local-image runtime inventory must remain bounded, long-path-safe, narrowly Windows-RECORD-compatible, extraction-root-safe, link-safe, collision-safe, path-free, non-overwriting, and unable to grant redistribution authority."
