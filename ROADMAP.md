...
Current status: The explicit bounded `.txt`/`.md`/`.csv`/`.json`, narrow source-text, and browsed-or-pasted PNG attachment slice is implemented with path-free selection, type/content checks, inert previews, compact scrolling, removal, memory-only cleanup, warned private-network transfer, and native Windows plus Ubuntu evidence. Physical macOS clipboard validation remains owner-parked. The deterministic lexical-retrieval core and optional conversation-history lifecycle/encryption architecture remain offline and inactive with no route, database, provider, or persistence authority. A 27-case metadata-only parser boundary covers PDF, Office Open XML, and OpenDocument candidate identities. The exact ignored `pypdf` 6.14.2 artifact is exercised only by an offline review worker that passed 61 corpus security, 64 static, and 40 contract-parity/package-exclusion checks with Windows and Ubuntu source evidence. A production-isolation assessment keeps missing OS sandbox controls explicit, metadata-only corpus research selects no artifact, and the intake/parity gates remain offline and false. The bounded standard-library Office/OpenDocument foundation passes 41 container checks across 16 fixtures plus 44 semantic checks across 12 fixtures with Windows and Ubuntu source evidence, without archive extraction or a dependency. Three deterministic PDF compliance files are generated only beneath ignored local review. No user document, dependency, application route, UI, provider path, package, installer, updater, or release uses either prototype. macOS source evidence, admitted non-synthetic hostile review, implemented production isolation, full semantic fidelity, actual package compliance integration, source/package parity, native package smoke, and explicit admission remain open. Vision promotion, broader image conversion, folder scans, active retrieval, embeddings, encrypted history, and runtime persistence remain unadmitted.
...
9. Add non-macro `.docx`/`.xlsx`/`.pptx` extraction only after independent Open XML package, relationship, formula, presentation-semantics, dependency, and native gates pass. The shared candidate-only ZIP/XML inspection foundation passes 41 checks across 16 deterministic containers. A standard-library semantic prototype adds 44 checks across 12 six-format fixtures, rejects formulas and over-budget parts/text, and passed source orchestration on Windows and Ubuntu Linux without archive extraction or third-party dependencies. Table/shared-string/comment/change/note/chart/header/footer semantics, non-synthetic evidence, dependency selection, production isolation, package parity, and runtime admission remain open.
