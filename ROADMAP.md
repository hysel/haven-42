...
4. Add trusted citation rendering and per-query disclosure without model-supplied links. The dormant fixed-Wikipedia renderer and its exact source-accounting validator are done: rendering is text-only, full destinations remain inactive, source counts are announced, New task clears the region, and static plus real-browser checks cover malformed, forged, duplicate, active, and markup-bearing inputs. No runtime route invokes it; accessible explicit navigation and its separate approval remain unimplemented.
5. Add explicit user-selected page retrieval with SSRF revalidation, strict content limits, inert text extraction, and no page execution. A development-only fixed-Wikipedia source command now binds one fresh exact query result, engine citation, and destination to two-pass public DNS validation, pinned TLS, strict bounded JSON/plain-text extraction, and no redirects, credentials, cookies, proxy inheritance, execution, files, or follow-up. It passes 41 hostile checks plus matching Windows/Linux live-source evidence and residue-free Linux cleanup. Accessible user review, runtime integration, hostile local-server evidence, macOS, native packages, and promotion remain unimplemented.
