...
Every unchecked `TODO.md` item is covered exactly once by a fail-closed machine-readable closure ledger. It distinguishes local work from external evidence, upstream dependencies, owner decisions, prerequisite admissions, and signing or release gates so implementation cannot silently drift between milestones.
...
| Milestone 23: Native Local Image Generation | In progress | The Linux ComfyUI/SDXL profile is live-validated and promoted; a 28-case effect-free lifecycle planner covers install, update, failed-health recovery, rollback, retention, interruption, and uninstall without granting machine authority. Windows NVIDIA/Quadro RTX 5000 includes an exact ComfyUI v0.30.0 side-by-side update and v0.29.2 rollback pass; Windows AMD/RX 7800 XT includes an exact v0.30.0 update and v0.28.0 rollback pass; and Windows Intel/Arc B580 passes an exact v0.30.0 XPU core cell plus a v0.29.2 rollback and v0.30.0 forward-selection transition. A fail-closed offline license auditor verified the retained AMD v0.30.0 archive, assigned all 448 native files to exact wheel records or the contracted embedded CPython runtime, and found no wheel-recorded native hash mismatch; unresolved license evidence and native component terms still block redistribution. NVIDIA and Intel exact archive audits remain pending. Consumer onboarding, automatic idle shutdown, complete uninstall/package parity, explicit redistribution approval, and Apple Silicon remain gated. |
