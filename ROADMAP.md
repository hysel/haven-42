...
| Milestone 28 | Active. One manual, fixed-Wikipedia query and selected-page path is admitted in source; package parity, assistive-technology review, cited synthesis, and broader providers remain gated. |
...
| Milestone 28 — Web research | Validate the owner-approved manual fixed-Wikipedia query/page slice without expanding model or navigation authority. | May ship only after exact package parity and accessibility gates pass; cited synthesis remains separate. | Broader providers, multi-query work, and active navigation require independent owner-approved capability gates. |
...
6. Keep Milestone 28 limited to the owner-approved manual fixed-Wikipedia
...
| Milestone 28: Controlled Web Research | In progress; manual fixed-provider source runtime admitted | Chat can prepare and separately approve one exact English Wikipedia search and one selected-page read through server-owned single-use tokens. Fixed-host transport, inert rendering, memory cleanup, hostile runtime/API checks, and real-browser coverage pass in source. Model invocation, model tools, active navigation, autonomous follow-up, persistence, downloads, cited synthesis, signed release status, and full native package/accessibility parity remain unadmitted. |
...
Current status: Active and narrowly admitted in source. Chat can prepare one bounded English Wikipedia metadata search, show the exact query/provider before any request, and consume a server-owned five-minute single-use approval. Returned titles, retrieval times, domains, and complete destinations remain inactive text. Reading one selected page requires a separate exact review and approval; the engine repeats metadata validation, derives the numeric page identity from the engine citation, and renders only bounded inert text. The fixed transports use system TLS trust, two public-DNS passes, a pinned public connection address, identity encoding, fixed host/path/parameters, strict JSON and byte/time limits, and no redirects, proxy inheritance, credentials, or cookies. New task and shutdown clear memory. The query transport passes 23 checks, page transport 41, runtime/API boundary 40, citation renderer 40, approval UI 64, local-web server 460, and the full Windows headless Chromium flow 622. Earlier sanitized source live runs passed on Windows and native Linux. This does not admit model invocation, model tools, active links, browser navigation, autonomous follow-up, persistence, downloads, page execution, cited synthesis, or broader providers. Exact native package parity and the manual accessibility matrix remain open.
...
1. Define versioned search-query, result, citation, provider-disclosure, research-budget, and memory-lifecycle contracts with network and follow-up effects denied by default. Done: the initial offline foundation remains the fail-closed base, while the product contract admits only the manual fixed-Wikipedia query/page path and continues to deny model tools, navigation, persistence, downloads, and follow-up.
...
3. Implement one explicit query-only search adapter returning bounded titles, domains, inactive public HTTPS destinations, and retrieval times. Done for the fixed-Wikipedia adapter and manual product route: its request/response adapter passes 15 checks, its native transport passes 23 hostile checks, and earlier sanitized Windows and native Linux live queries passed. Models and automatic follow-up cannot invoke it.
4. Add trusted citation rendering and per-query disclosure without model-supplied links. Done for server-validated fixed-Wikipedia bundles. Rendering is text-only, destinations remain inactive, source counts are announced, New task clears the regions, and static plus real-browser checks cover hostile shapes, focus, cancellation, exact disclosure, trusted single-use approval, short viewports, and background interaction. Active navigation remains a separate unimplemented gate.
