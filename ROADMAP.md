...
The repository is in early implementation stage. Milestones 1 through 21 are complete for their defined supported-surface, generated-fixture, workflow-foundation, and general-capability scopes. Continue, Aider, and OpenCode form the maintained surface set; failed or retired integrations are removed rather than carried as partial implementations. OpenHands remains a documentation-only candidate with a defined but unimplemented isolation boundary. Milestone 22 is in progress with its Tauri/private-IPC desktop architecture selected; implementation remains gated by dependency, security, packaging, and cross-platform evidence. Milestone 23 has a promoted Linux image profile and partial Windows AMD evidence. Milestone 24 has partial Linux CUDA ACE-Step evidence, while Milestone 25 remains documentation-only; neither ships an executable integration. Milestone 26 has its foundation, exact Linux/NVIDIA/Ollama and Windows/AMD/Ollama trusted-artifact comparisons, and a provider/engine/backend abstraction. llama.cpp CUDA passed the exact Linux NVIDIA cell and HIP passed the exact Windows AMD cell while Vulkan failed the patch gate; Intel work is parked pending representative hardware, and conversion, activation, broader workloads, Windows NVIDIA, and Apple Silicon remain open.
...
| Milestone 22: Unified Product UI And Task Composition | In progress | Tauri 2, private typed IPC, cross-platform storage, immutable update/rollback contracts, workflow reliability, a security threat model, local-data lifecycle rules, a 46-case offline sidecar suite, and hostile non-activating updater tests are defined. Native security/package gates remain, so runtime files stay out of the repository. |
