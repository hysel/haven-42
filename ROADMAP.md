...
| Milestone 28: Controlled Web Research | Proposed; runtime unadmitted | Default-deny contracts plus offline result, approval-review, page-text, citation, and cited-synthesis hostile suites exist, but no model invocation, model tool, product invocation, DNS, network, page retrieval, active citation, or autonomous follow-up authority is admitted. |
...
Current status: Proposed and security-scoped. Development-only native fixed-Wikipedia metadata-query and selected-page commands now have sanitized Windows and native headless Linux source passes. The query transport passes 21 hostile offline checks. The selected-page transport adds 41 checks, requires a fresh exact engine citation and destination before one page request, accepts only a strict bounded JSON plain-text extract, and produced the same content digest on both platforms. Each Linux run used a hash-listed disposable source bundle and left no matching temporary directory. Both commands use system TLS trust, two-pass public DNS validation, a pinned public connection address, identity encoding, fixed hosts and paths, strict JSON and byte/time limits, and no redirects, proxy inheritance, credentials, or cookies. The transport commands remain absent from the app runtime and packages. Two separate dormant UI foundations are packaged with the existing chat source but have no invoking route. The trusted renderer accepts only exact engine-accounted fixed-Wikipedia citations, creates text nodes rather than model Markdown or links, discloses full inactive destinations, announces status accessibly, and clears on New task. The explicit review dialog presents the exact query/provider or page/provider/source/destination, traps and returns focus, makes the background inert, supports Cancel, close, Escape, and one trusted Approve once action, preserves focus during passive cleanup, and keeps its frozen decision memory-only and single-use. The renderer's 39 static plus 19 browser checks and the review dialog's 63 static plus 43 browser checks grant no network, model-tool, active-navigation, persistence, page execution, download, telemetry, or follow-up authority. The wider foundation still includes ten inert hostile fixtures, a 28-check caller-fixture boundary, 15 fixed-adapter request/response checks, a 25-check effect-free transport guard, a 17-check memory-only approval proof, a 26-check caller-bytes-only page-text foundation, and a separate 26-check cited-synthesis boundary. Self-hosted-provider and at-most-four-query evaluation contracts remain inactive. Text models retain no general research authority, and the existing fixed-origin Ollama model-catalog search remains a separate candidate-discovery feature.
...
4. Add trusted citation rendering and per-query disclosure without model-supplied links. The dormant fixed-Wikipedia renderer, exact source-accounting validator, and explicit query/page review dialog are done. Rendering is text-only; full destinations remain inactive; source counts are announced; New task clears the regions; and static plus real-browser checks cover malformed, forged, duplicate, active, markup-bearing, synthetic-approval, focus, cancellation, caller-mutation, and background-interaction cases. No product route invokes either component. The review decision is single-use and effect-free; accessible explicit navigation and runtime integration remain unimplemented.
5. Add explicit user-selected page retrieval with SSRF revalidation, strict content limits, inert text extraction, and no page execution. A development-only fixed-Wikipedia source command now binds one fresh exact query result, engine citation, and destination to two-pass public DNS validation, pinned TLS, strict bounded JSON/plain-text extraction, and no redirects, credentials, cookies, proxy inheritance, execution, files, or follow-up. It passes 41 hostile checks plus matching Windows/Linux live-source evidence and residue-free Linux cleanup. Dormant accessible user review is present but unconnected; runtime integration, hostile local-server evidence, macOS, native packages, and promotion remain unimplemented.
