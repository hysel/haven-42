...
Current status: The explicit `.txt`/`.md`/`.csv`/`.json` plus browsed-or-pasted PNG attachment slice is implemented with one type-restricted picker, strict atomic count/byte/type/dimension/structured-text validation, inert format-aware previews, compact attachment scrolling that preserves the chat composer, removal, memory-only cleanup, warned submit-confirmed private-network transfer, and visible unverified image-input status. Sanitized Windows source and unsigned packaged/default-browser cells passed native screenshot paste, mixed file selection, user-reviewed `qwen3.5:9b` description, chat-layout review, and independently verified model unload; expanded CSV/JSON native package parity awaits the next hosted package run. PNG is the only admitted screenshot file-picker and clipboard representation and the canonical provider payload. Native Linux clipboard validation is the next physical target and requires a real desktop/browser clipboard session; headless synthetic events are insufficient. Physical macOS clipboard validation is owner-parked until suitable hardware is acquired. The deterministic lexical-retrieval core is implemented and hostile-tested offline with bounded casefolded term-frequency ranking, stable attachment/offset tie-breaking, token and selected-chunk disclosure, removal, clear-all, and failure cleanup. It remains inactive: no runtime route, UI control, provider payload, path, parser, network, embedding, or persistence authority is admitted. The optional conversation-history foundation now has a versioned default-deny contract, logical SQLite-compatible schema with no executable SQL, pure migration/retention/context/deletion/recovery/backup/restore planners, and 16 inert hostile fixtures. Its encryption/key-management architecture review requires a user-scoped OS credential facility and fail-closed Private session with no plaintext fallback; no dependency, database, key, route, or write is admitted. A 24-case restricted parser-worker admission foundation rejects hostile PDF/Office metadata without opening a document, importing a parser, starting a worker, or granting a route. All database, file, browser-storage, network, process, provider, parser-worker, and machine effects remain false. Vision-model recommendation evidence, non-PNG clipboard conversion, broader image upload, folder scans, real complex document parsers, active retrieval integration, embeddings, encrypted conversation storage, and runtime persistence remain unadmitted.
...
4. Run native clipboard-paste smoke tests on the admitted Windows, Linux, and
   ...
   representation. Sanitized Windows source and unsigned packaged/default-
   browser cells are done. Linux is the next physical target, provided the
   available machine exposes a real desktop/browser clipboard session; physical
   macOS is owner-parked until hardware is acquired. Keep unsupported
   representations rejected with a clear warning.
...
6. Extend inert plain-text selection to CSV and JSON, then separately reviewed source-code extensions, with format-aware previews and the existing no-execution boundary. Done for bounded UTF-8 CSV/JSON with client and engine syntax/resource validation, inert format-aware previews, no formula evaluation, and the existing memory-only provider boundary; source-code extensions remain separate.
7. Introduce the restricted parser-worker contract and hostile parser harness before admitting any complex document parser. Done at the offline admission boundary with 24 cases covering paths, authority fields, size/object/nesting/expansion budgets, encryption, active content, macros, external relationships, embedded objects, and exact worker limits. No parser dependency, worker, route, filesystem access, or runtime authority is admitted.
