...
5. Use the on-demand runtime certification workflow for Ollama, llama.cpp, and
   each later admitted engine. Discovery may identify a newer official release,
   but downloads, native tests, compatibility-registry changes, managed
   defaults, support labels, and promotion remain separately gated.
6. Advance Milestones 23–27 only through their independent hardware, security,
...
- Poll reviewed official publisher namespaces by modification time in addition to search queries, retain immutable revisions, and maintain a candidate-only release radar. Done for the 2026-08-16 sweep; the configured 45-day publisher window can no longer be narrowed by a manual timestamp, and a newest-first Ollama index adapter discovers unfamiliar registry families without seed names. Qwen 3.8, Gemma 4, compact assistant, multilingual, translation, vision/video, safety, embedding, speech, document-vision, computer-use, and coding candidates are queued without downloads, tests, promotion, or default changes.
...
Current status: Active and narrowly admitted in source. Chat can prepare one bounded English Wikipedia metadata search, show the exact query/provider before any request, and consume a server-owned five-minute single-use approval. Returned titles, retrieval times, domains, and complete destinations remain inactive text. Reading one selected page requires a separate exact review and approval; the engine repeats metadata validation, derives the numeric page identity from the engine citation, and renders only bounded inert text. A separately approved multi-source path accepts a session-only Brave Search key, validates at most five returned public HTTPS destinations, retrieves bounded inert text with fresh DNS checks and pinned connections, and asks the selected local Ollama model for strict citation-bound claims. The key is cleared after use or failure and is never saved or sent to the model. A browser fallback still opens one exact reviewed Brave Search destination without ingestion. New task and shutdown clear in-app research memory. The dedicated multi-source boundary passes 21 hostile checks; the shared runtime/API boundary passes 48 and the local-web server passes 479. This does not admit model initiation or approval, model tools, arbitrary URLs, autonomous follow-up, persistence, downloads, page execution, or active citation navigation. Exact native package parity, the manual accessibility matrix, and updated complete browser coverage remain open.
