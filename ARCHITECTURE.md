...
The renderer maintains a single-primary-panel invariant across Chat, Software, Images, Models, and About. Models owns capability-specific installed selection and candidate discovery; the provider connection remains a separate configuration surface. System mirrors the engine-approved immediate, 5-minute, 15-minute, and 30-minute model-residency choices and applies a change only through an explicit reconnect using the validated endpoint. About is informational only. Keyboard submission maps Enter to the existing bounded text form and reserves Shift+Enter for multiline input, adding no endpoint or execution authority.
