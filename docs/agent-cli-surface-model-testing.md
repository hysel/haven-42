...
| Kilo Code | `scripts/test-kilo-code-cli-models.ps1` | `scripts/test-kilo-code-cli-models.linux.sh`, `scripts/test-kilo-code-cli-models.macos.sh` | `kilo run --auto --agent code --format json --model "ollama/{Model}" "{Prompt}"`; generate `.kilo.local.json` through the unified adapter. The explicit `code` agent prevents Kilo's conversational default agent from bypassing repository work. The harness passes that exact config through `KILO_CONFIG` for every live command, including write and scoped-edit validation. |
...
- **Kilo Code:** The official project documents `npm install -g @kilocode/cli` and `kilo run --auto "{Prompt}"`, with `--model provider/model` and `--format json`. This pack also selects Kilo's `code` agent explicitly; its default `ask` agent is conversational rather than an appropriate validation target for repository edits. Generate `.kilo.local.json` through the unified adapter. The Kilo test wrappers require this config and pass it through `KILO_CONFIG` automatically, then keep real-project approved-write blocked until explicitly approved non-generated repository validation passes.
