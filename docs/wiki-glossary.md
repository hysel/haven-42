...
Haven 42 explains unfamiliar words where they first appear. Use this page when
...
| Token | A small piece of text processed by the model. Token totals describe response length; tokens per second describe speed. |
...
| GPU | A graphics processor that can accelerate a compatible AI model. Its available graphics memory limits which model sizes fit. |
...
| Soak test | A test that runs for a set period to catch failures that may not appear in a short check. |
