...
| Native macOS 26.6.2, physical Mac | Apple M4, 16 GB unified memory | llama.cpp `b10520` commit cd644c395 with Metal | ⚠️ Partial | The pinned Qwen 3.5 0.8B GGUF passed full-layer Metal offload, authenticated loopback inference, recovery, restart, unload, and cleanup. The exact official arm64 archive also passed digest, safe-extraction, relocation, and dependency-free launch checks. Its executable is ad-hoc signed and rejected by Gatekeeper, so trusted public packaging and a maintained coding surface remain open. |
