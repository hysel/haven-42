...
Runtime validation now injects filename-fidelity instructions at execution time in addition to prompt-level guidance. The verifier remains the acceptance gate because the tested local model can still invent conventional filenames after receiving those instructions.
