...
`scripts/simulate-task-composition.py` performs deterministic topological planning. Cancellation can stop the plan before any step artifact is emitted. Results always set `executionAllowed` to false and explicitly report process creation, filesystem access, network access, and machine modification as false.
