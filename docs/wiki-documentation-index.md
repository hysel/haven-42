# Advanced Documentation Index

_For contributors, maintainers, security reviewers, and evaluators._

You do not need these pages to set up or use Haven 42. Start with
[[Quick Start|Quick-Start]], [[Using Haven 42|Using-Haven-42]], or the
[[Glossary|Glossary]].

This index keeps detailed engineering material available to contributors
without placing more than one hundred development pages in the main user
sidebar. GitHub's wiki page list remains the complete alphabetical inventory.

## Product and architecture

- [[Solution Architecture Review|Solution-Architecture-Review]]
- [[Local Web MVP|Local-Web-MVP]]
- [[Product UI First Slice|Product-UI-First-Slice]]
- [[Capability Registry|Capability-Registry]]
- [[Workflow Registry|Workflow-Registry]]
- [[Bounded Task Composition|Task-Composition]]
- [[Desktop IPC Contract|Desktop-IPC-Contract]]
- [[Desktop Storage and Updates|Desktop-Storage-And-Updates]]

## Security, privacy, and lifecycle

- [[Security Threat Model|Security-Threat-Model]]
- [[Provider Endpoint Security|Provider-Endpoint-Security]]
- [[Local Data Lifecycle|Local-Data-Lifecycle]]
- [[Onboarding Setting Security|Onboarding-Setting-Security]]
- [[Conversation History Database Foundation|Conversation-History-Database]]
- [[Conversation History Encryption Review|Conversation-History-Encryption-Review]]
- [[Post-Quantum Cryptography Readiness|Post-Quantum-Cryptography-Readiness]]
- [[Restricted Parser-Worker Foundation|Restricted-Parser-Worker-Foundation]]
- [[PDF Production Isolation|PDF-Production-Isolation]]
- [[Controlled Web Research Foundation|Controlled-Web-Research-Foundation]]
- [[Retrieval Expansion Foundations|Retrieval-Expansion-Foundations]]

## Models, providers, and media

- [[Engineering Model Selection|Engineering-Model-Selection]]
- [[Image Provider Admission|Image-Provider-Admission]]
- [[Local Image Runtime License Review|Local-Image-Runtime-License-Review]]
- [[Local Text Capabilities|Local-Text-Capabilities]]
- [[Local Model Reliability|Local-Model-Reliability]]
- [[Model Tool Use Validation|Model-Tool-Use-Validation]]
- [[Local Image Capability|Local-Image-Capability]]
- [[ComfyUI Image Provider Setup|ComfyUI-Image-Provider-Setup]]
- [[Local Audio Provider Candidates|Local-Audio-Provider-Candidates]]
- [[Local Video Provider Candidates|Local-Video-Provider-Candidates]]
- [[Generative Media Consent Policy|Generative-Media-Consent-Policy]]
- [[Inference Engine Validation|Inference-Engine-Validation]]
- [[Intel B580 Inference Engine Validation|Intel-B580-Inference-Engine-Validation]]
- [[Cross-Accelerator Model Validation|Cross-Accelerator-Model-Validation]]

## Agent and engineering workflows

- [[Coding Tools for Local Models|Coding-Tools-For-Local-Models]]
- [[Agent Integration Admission Policy|Agent-Integration-Admission-Policy]]
- [[Agent Surface Promotion Gates|Agent-Surface-Promotion-Gates]]
- [[Agent Surface Capability Parity|Agent-Surface-Capability-Parity]]
- [[Workflow Chooser|Workflow-Chooser]]
- [[Tool Use Modes|Tool-Use-Modes]]
- [[Approved Tool-Backed Changes|Approved-Tool-Backed-Changes]]
- [[Scoped Edits|Scoped-Edits]]
- [[Editor Compatibility|Editor-Compatibility]]
- [[Language Support|Language-Support]]
- [[Project Detection|Project-Detection]]
- [[Language Rule Packs|Language-Rule-Packs]]
- [[Public Repository Validation Candidates|Public-Repository-Validation-Candidates]]

## Packaging, testing, and release evidence

- [[Arch Linux Test VM Installation|Arch-Linux-Test-VM-Installation]]
- [[Engineering Evidence Dashboard|Engineering-Evidence-Dashboard]]
- [[Evidence Catalog|Evidence-Catalog]]
- [[Evidence Dashboard|Evidence-Dashboard]]
- [[Capability Evidence Contract|Capability-Evidence-Contract]]
- [[Runtime Validation|Runtime-Validation]]
- [[Runtime Output Verification|Runtime-Output-Verification]]
- [[Hosted CI Verification|Hosted-CI-Verification]]
- [[Test Tiers|Test-Tiers]]
- [[Unsigned Development Artifact Attestation|Artifact-Attestation]]
- [[Release Guidance|Release-Guidance]]
- [[Milestone 22 Admission Readiness|Milestone-22-Admission-Readiness]]
- [[Windows Alpha Native Validation|Windows-Alpha-Native-Validation]]

## Project management and maintenance

- [[Engineering Roadmap|Engineering-Roadmap]]
- [[Engineering Troubleshooting|Engineering-Troubleshooting]]
- [[Project Status Consistency|Project-Status-Consistency]]
- [[Local Batch Task Ledger|Local-Batch-Task-Ledger]]
- [[GitHub Repository Policy|GitHub-Repository-Policy]]
- [[Wiki Maintenance|Wiki-Maintenance]]

Detailed `*-Validation` pages remain exact, scope-limited evidence. They should
not be interpreted as general availability or production-readiness claims.
