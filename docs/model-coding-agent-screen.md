...
Apply the same coding-agent screen to every executable local-model candidate,
...
Each result applies to one model digest, runtime artifact, hardware profile,
editor or CLI surface, and surface version. Results do not carry across VS
Code, VSCodium, native chat, or an extension. Every gate and
...
scoped-write, external-diff, and unintended-write checks. Results do not
...
Do not include raw prompts, raw responses, private endpoints, machine identity,
or network identity in the sanitized cell. Treat unintended writes as a failed
