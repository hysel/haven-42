...
Start with one non-Continue surface in read-only mode. Cline has a recorded editor evidence path in `docs/cline-readonly-validation.md`; Aider has the first supported non-Continue setup adapter through `docs/aider-cli-model-testing.md`; shared CLI-surface screening is documented in `docs/agent-cli-surface-model-testing.md`.
