...
| Cline | VS Code-compatible editor agent | Read-only validated for generated Python sample with `qwen3-coder:30b` at 16k context | Read-only validation guide and sanitized evidence available | Blocked | Use `docs/cline-readonly-validation.md` and `examples/cline-readonly-validation.md`; approved-write remains blocked until a scoped write test passes external verification. |
