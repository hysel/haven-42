...
| 26: Hardware-Adaptive Model Quantization | Engine evidence expanded | Foundations and three hardware families have evidence | Hardware profiling, trusted-artifact selection, format/runtime boundaries, Ollama comparisons on Linux NVIDIA and Windows AMD, and llama.cpp CUDA/HIP engine evidence are complete for their exact cells. Physical Linux Intel Arc B580 candidate evidence now covers llama.cpp SYCL and OpenVINO GenAI, but upstream test, OS support, strict-output, provider, and package blockers prevent promotion. Conversion, activation, rollback, Windows Intel, Apple Silicon, and UI integration remain open. |
| 27: Local Knowledge Context And Retrieval | In progress; bounded attachments admitted and offline retrieval/history/parser foundations effect-free | Unified bounded `.txt`/`.md`/`.csv`/`.json`/PNG selection, clipboard PNG paste, deterministic memory lexical core, encryption/key architecture review, default-deny conversation-history planning, and restricted parser admission implemented | Explicit path-free selection, strict atomic validation, structured-text syntax/resource checks, inert format-aware previews, warned private-network transfer, and cleanup are implemented. The offline lexical engine has bounded ranking, disclosure, removal, and hostile lifecycle tests but exposes no route, UI, provider payload, path, or persistence. History adds a non-executable schema and pure lifecycle planners while opening no database; OS-key handling must fail closed without plaintext fallback. The review-only PDF worker has Windows/Ubuntu source evidence, explicit production-isolation gaps, a 32-check exact OS-control evidence gate, metadata-only corpus research with zero selected artifacts, and false package parity. The standard-library complex-document foundation passes 41 container checks across 16 fixtures, 57 semantic checks across 17 six-format fixtures, and 33 package-exclusion checks on Windows and Ubuntu source lanes, while dependencies, runtime, provider, UI, packages, macOS, and unsupported fidelity remain false. Vision promotion, admitted non-synthetic evidence, implemented production isolation, native package parity, folder scanning, embeddings, and runtime persistence remain unadmitted. |
...
- Tests requiring hardware or operating-system profiles not already validated,
  including Windows Intel and final physical-Mac release gates. Windows NVIDIA
  now has partial image-provider evidence, while Linux Intel Arc B580 hardware
  has candidate engine evidence that does not transfer to Windows.
