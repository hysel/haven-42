...
| 27: Local Knowledge Context And Retrieval | Proposed; runtime unadmitted | Security and staged-delivery boundary defined | Explicit browser file selection, memory-only initial processing, strict format/resource limits, private-network disclosure, prompt-injection isolation, retrieval, and cleanup gates are defined. No file-ingestion, folder scanning, embedding index, or persistence runtime ships. |
