...
| 26: Hardware-Adaptive Model Quantization | Engine evidence expanded | Foundations and three hardware families have evidence | Hardware profiling, trusted-artifact selection, format/runtime boundaries, Ollama comparisons on Linux NVIDIA and Windows AMD, and llama.cpp CUDA/HIP engine evidence are complete for their exact cells. Physical Intel Arc B580 candidate evidence covers Linux llama.cpp SYCL plus Linux and Windows OpenVINO GenAI, but upstream tests, Linux OS support, strict-output, provider, and package blockers prevent promotion. Conversion, activation, rollback, Apple Silicon, and UI integration remain open. |
...
| 28: Controlled Web Research | Proposed; runtime unadmitted | Security, staged-delivery, offline adapter/citation validation, and caller-bytes extraction boundaries defined | Explicit reviewed queries, engine-owned fixed adapters, trusted citations, SSRF/redirect/content controls, prompt-injection isolation, source accounting, and memory cleanup gates are defined. A 28-check caller-fixture boundary validates strict result metadata and inactive citations without a network stack. A separate 26-check caller-bytes-only boundary emits inert text from bounded UTF-8 plain text or allowlisted HTML without attributes, remote references, files, runtime, package, UI, or model authority. Models and the renderer have no internet tool, and no live adapter or page transport exists. |
