...
| 27: Local Knowledge Context And Retrieval | In progress; initial attachment admitted and offline retrieval/history foundations effect-free | Unified bounded `.txt`/`.md`/PNG selection, clipboard PNG paste, deterministic memory lexical core, encryption/key architecture review, and default-deny conversation-history planning implemented | Explicit path-free selection, strict atomic validation, compact previews, warned private-network transfer, and cleanup are implemented. The offline lexical engine has bounded ranking, disclosure, removal, and hostile lifecycle tests but exposes no route, UI, provider payload, path, or persistence. History adds a non-executable schema and pure lifecycle planners while opening no database; OS-key handling must fail closed without plaintext fallback. Vision promotion, native retrieval parity, dependency selection, folder scanning, broader parsers, embeddings, and runtime persistence remain unadmitted. |
