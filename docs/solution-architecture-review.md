...
| 27: Local Knowledge Context And Retrieval | In progress; initial text/screenshot attachment admitted | Unified bounded `.txt`/`.md`/PNG selection, clipboard PNG paste, provider disclosure, and inactive lexical contract implemented | Explicit path-free selection, strict atomic text/PNG structural and resource validation, keyboard access, compact attachment scrolling, safe previews, removal, warned submit-confirmed private-network transfer, untrusted-context labeling, visible unverified image-input status, memory-only cleanup, packaged-browser matrix coverage, and hostile source/browser tests are implemented. The lexical foundation exposes no route or provider payload. Vision-model promotion, retrieval implementation, folder scanning, broader parsers, embeddings, and persistence remain unadmitted. |
