...
| 28: Controlled Web Research | In progress; manual fixed-provider source runtime admitted | One explicit fixed-Wikipedia query and selected-page path is connected; package/accessibility/synthesis gates remain | Server-owned five-minute single-use query and page approvals bind exact review bundles to fixed native transports. The engine enforces two-pass public DNS, pinned TLS, fixed host/path/parameters, strict response budgets and shapes, engine citations, inert text rendering, and memory cleanup. The product boundary passes 40 hostile API checks and the full Windows headless Chromium flow passes 622 checks. Models cannot invoke or approve research, choose URLs, navigate, persist, download, or follow up. Exact native package parity, manual assistive-technology review, cited synthesis, active navigation, self-hosted providers, and multi-query work remain independently gated. |
