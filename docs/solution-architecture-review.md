...
| 28: Controlled Web Research | Proposed; runtime unadmitted | Security, staged-delivery, offline adapter/citation, caller-bytes extraction, and cited-synthesis boundaries defined | Explicit reviewed queries, engine-owned fixed adapters, trusted citations, SSRF/redirect/content controls, prompt-injection isolation, source accounting, and memory cleanup gates are defined. A 28-check general caller-fixture boundary and 15-check fixed-Wikipedia fixture-transport adapter validate strict request/result metadata and inactive destinations without a network stack. A 25-check effect-free transport guard covers DNS/IP snapshots, rebinding, destination, redirect, response type, encoding, time, and byte rules; a 17-check memory-only state proof covers exact single-use query/page approval and lifecycle cleanup. Separate 26-check page-text and 26-check cited-synthesis boundaries emit inert text, construct URL-free digest-accounted source envelopes, and require exact citations for bounded candidate claims. Self-hosted and bounded multi-query contracts select no provider and grant no authority. Models and the renderer have no internet tool, and no live adapter or page transport exists. |
