...
| 27: Local Knowledge Context And Retrieval | In progress; initial text/screenshot attachment admitted and history foundation effect-free | Unified bounded `.txt`/`.md`/PNG selection, clipboard PNG paste, provider disclosure, inactive lexical contract, and default-deny conversation-history planning implemented | Explicit path-free selection, strict atomic text/PNG structural and resource validation, keyboard access, compact attachment scrolling, safe previews, removal, warned submit-confirmed private-network transfer, untrusted-context labeling, visible unverified image-input status, memory-only cleanup, packaged-browser matrix coverage, and hostile source/browser tests are implemented. The lexical foundation exposes no route or provider payload. The history foundation adds a non-executable logical schema plus pure migration, retention, context, deletion, recovery, backup, and restore planners while opening no database and keeping all effects false. Vision-model promotion, retrieval implementation, folder scanning, broader parsers, embeddings, encryption/key decisions, and runtime persistence remain unadmitted. |
