...
| 23: Native Local Image Generation | In progress | One promoted profile, one partial native cell, and an effect-free lifecycle planner | Linux ComfyUI/SDXL is promoted. A strict 28-case planner models future install, update, failed-health recovery, rollback, retention, interruption, and uninstall only for promoted profiles with every effect false. Windows AMD/RX 7800 XT passed generation, visual, privacy, restart, cancellation, forced recovery, repeated-run, retention cleanup, and uninstall, but still requires a real immutable update/rollback candidate and consumer onboarding evidence. Windows NVIDIA, Windows Intel, and Apple Silicon remain open. |
...
- Tests requiring hardware not already available, including Windows NVIDIA,
  Windows Intel, and final physical-Mac release gates. Linux Intel Arc B580
  hardware is now available and has candidate engine evidence.
