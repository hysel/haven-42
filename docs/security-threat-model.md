...
Milestone 27 document context remains proposed and unadmitted. Its promotion gate requires explicit file selection, no arbitrary path or background scan authority, strict format and resource allowlists, memory-only lifecycle, exact provider-scope disclosure, private-network confirmation, prompt-injection isolation, and residue-free cross-platform source/package tests. PDF, Office, archive, OCR, directory, embedding, and persistent-index support do not inherit approval from plain-text attachment.

Milestone 28 controlled web research remains proposed and unadmitted. Its promotion gate requires explicit reviewed queries, engine-owned fixed adapters, no renderer/model destination control, DNS and resolved-IP revalidation, redirect and content limits, no page execution, trusted citation rendering, hostile-content prompt isolation, exact source accounting, and residue-free memory cleanup. The admitted fixed Ollama catalog search does not grant or imply general research-search authority.
