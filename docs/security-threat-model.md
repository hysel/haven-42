...
The local-web admission applies to read-only readiness inspection, zero-effect setup planning, status, exact-digest Ollama discovery and text, plan-only registered read-only software workflows, and the exact promoted Linux ComfyUI/SDXL image profile through loopback. The only admitted browser persistence is a fixed-key record containing five positive integer section-tour revisions. A stored revision means that exact section-tour revision was completed or dismissed; it contains no progress position, conversation, attachment, model, provider, hardware, credential, or identity data. The admission does not include other client persistence, workflow process execution, arbitrary provider profiles, installation, elevation, service or driver changes, updates, remote UI access, or Tauri packaging. No optional desktop runtime ships until actual Windows, Linux, and macOS binaries pass renderer, IPC, canonical-path, lifecycle, update, rollback, packaging, uninstall, privilege, and security tests. Unsupported or failed provider cells remain documentation-only and leave no executable integration.
