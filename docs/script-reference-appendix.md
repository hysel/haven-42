...
| Memory lexical retrieval test | Exercises the inactive deterministic in-memory engine with bounded ranking, source and truncation disclosure, hostile text, removal, and cleanup; performs no file, network, provider, or process operation. | `python scripts/test-memory-lexical-retrieval.py` | Same dependency-free Python command | `ROADMAP.md` Milestone 27 |
| Structured tool-transport test | Validates exact final Ollama 0.32.5 and normalized OpenAI-compatible candidate calls, rejects hostile envelopes and arguments, and grants no provider, approval, execution, or runtime authority. | `python scripts/test-structured-tool-transport.py` | Same dependency-free Python command | `docs/structured-tool-transport-foundation.md` |
| Structured tool live validation | Requires explicit `--live`, uses a fixed synthetic call, validates only installed models, retains sanitized ignored evidence, downloads nothing, and attempts model unload after each cell. | `python scripts/run-structured-tool-live-validation.py --live --endpoint <IP URL> --trust-scope <scope> --models <models>` | Same dependency-free Python command | `docs/structured-tool-transport-foundation.md` |
| Native Linux parser-isolation probe | Reads normalized native capability availability without installation, privilege, host identity, or admission; WSL2 remains non-native and all implementation/parity fields stay false. | Not native Windows evidence | `python scripts/probe-linux-parser-isolation.py` | `docs/restricted-parser-worker-foundation.md` |
| Local image lifecycle simulation | Exercises strict install/update/rollback/recovery/retention/uninstall planning only for promoted image profiles; every machine effect remains false. | `python scripts/test-local-image-lifecycle-foundation.py` | Same dependency-free Python command | `ROADMAP.md` Milestone 23 |
| Restricted parser-worker admission | Exercises metadata-only PDF/Office hostile cases without opening a document, importing a parser, starting a worker, or writing a file. | `python scripts/test-parser-worker-foundation.py` | Same dependency-free Python command | `ROADMAP.md` Milestone 27 |
...
| Evidence input binding and freshness check | Records canonical SHA-256 bindings for exact repository inputs, then fails closed if a later importer sees missing, changed, duplicate, or escaped inputs. It grants no runtime or release authority. | `python scripts/alpha2-evidence-binding.py --repository-root . --output <binding.json> <role>=canonical-json=<path> ...` | Same dependency-free Python command | `docs/evidence-input-binding.md` |
| Hardware qualification importer and publication preflight | Imports sanitized validator output into the generic hardware evidence contract, checks exact fresh inputs, and reports publication blockers without promoting a model or changing product policy. | `python scripts/alpha2-hardware-qualification-evidence.py ...` and `python scripts/alpha2-hardware-report-preflight.py ...` | Same dependency-free Python commands | `docs/hardware-qualification-publication-workflow.md` |
| Hardware cross-OS comparison | Compares two generic, source-bound hardware result cells and refuses incomplete, stale, mismatched, or forged inputs. | `python scripts/alpha2-hardware-cross-os-report.py --first-result <json> --second-result <json> --output <json>` | Same dependency-free Python command | `docs/hardware-qualification-publication-workflow.md` |
| Model recommendation explanation | Explains why a model is eligible or rejected for one sanitized hardware profile while preserving runtime, evidence, memory, and capability gates. It downloads nothing and changes no default. | `python scripts/alpha2-model-recommendation-report.py ...` | Same dependency-free Python command | `docs/model-recommendation-explanations.md` |
| Coding-agent qualification screen | Validates one exact model, runtime artifact, hardware profile, editor surface, and surface version against every required coding gate. Missing gates remain blocked or not-run. | `python scripts/model-coding-agent-screen.py --policy <policy.json> --cell <cell.json>` | Same dependency-free Python command | `docs/model-coding-agent-screen.md` |
