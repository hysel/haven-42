...
| Memory lexical retrieval test | Exercises the inactive deterministic in-memory engine with bounded ranking, source disclosure, hostile text, removal, and cleanup; performs no file, network, provider, or process operation. | `python scripts/test-memory-lexical-retrieval.py` | Same dependency-free Python command | `ROADMAP.md` Milestone 27 |
