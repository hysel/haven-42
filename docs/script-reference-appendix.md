...
| Structured tool-transport test | Normalizes exact offline Ollama and OpenAI-compatible candidate calls, rejects hostile shapes and arguments, and grants no provider, approval, execution, or runtime authority. | `python scripts/test-structured-tool-transport.py` | Same dependency-free Python command | `docs/structured-tool-transport-foundation.md` |
