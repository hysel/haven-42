...
- PDF production isolation — the assessment records missing OS-enforced controls instead of treating source success as admission. Metadata-only research selected no external PDF, and the offline intake verifier rejects mutable, unlicensed, private, unreviewed, or unsafe-retention records.
- Complex-document semantics — a dependency-free review prototype passed 44 checks across 12 DOCX/XLSX/PPTX/ODT/ODS/ODP fixtures on Windows and Ubuntu Linux after the 41-check container gate. Formula handling, unsupported semantics, runtime, provider, UI, package, and macOS cells remain blocked.
