...
- Milestone 27 — one keyboard-operable bounded UTF-8 `.txt`/`.md`/`.csv`/`.json`/PNG picker plus clipboard PNG paste is implemented with structured-text syntax/resource checks, inert format-aware previews, compact scrolling, explicit path-free selection, warned submit-confirmed private-network transfer, visible unverified image-input status, and memory-only cleanup. Screenshot selection defaults to two per task with an advanced one-through-four choice, while the engine retains absolute four-image, 8-MiB combined, and 33.5-million combined decoded-pixel ceilings. The deterministic lexical core is implemented and hostile-tested offline but grants no runtime route, UI, provider, path, or persistence authority. The optional history foundation remains write-free and opens no database; its encryption/key architecture requires a current-user OS credential facility and fails closed without plaintext fallback. A 24-case restricted parser-worker foundation rejects hostile PDF/Office metadata while admitting no parser dependency or worker. Expanded native parity, dependency selection, vision promotion, automatic scanning, real complex parsers, embeddings, and runtime persistence remain unadmitted.
