...
Contributors can build the development ZIP from the Haven 42 repository:

```text
python packages/local-llm-ide/build.py
```

Extract the ZIP written under `dist/local-llm-ide`, then open a terminal in
the extracted folder. End users should receive the ZIP and its `.sha256` file;
they should not need to clone the full Haven 42 repository.
