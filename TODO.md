...
- [x] Implement one explicit query-only adapter that sends only the reviewed bounded query and returns strict result metadata.
  ...
  - [x] Add a dormant explicit query/page review dialog with exact provider and destination disclosure, trusted single-use approval, cancellation and Escape handling, focus containment and return, background inertness, passive-cleanup focus preservation, New task cleanup, forced-colors and reduced-motion support, and 63 static plus 43 real-browser checks. No product route invokes it, and its decision starts no network, navigation, runtime, model-tool, persistence, telemetry, or follow-up action.
  - [x] Add a development-only native fixed-Wikipedia query transport with system TLS trust, two-pass public DNS validation, connection pinning, redirect/compression/proxy/credential/cookie denial, strict response limits, 21 hostile offline checks, and sanitized Windows and native headless Linux live-query passes. The Linux proof used a hash-verified disposable bundle and left no matching temporary directory. Runtime, UI, model-tool, page-fetch, follow-up, persistence, and package authority remain false.
- [x] Add trusted citation rendering with destination disclosure and no model-supplied active links.
  ...
  - [x] Add a dormant fixed-Wikipedia renderer that accepts only the exact engine-accounted citation bundle, creates text nodes only, discloses full inactive destinations, announces source counts, supports forced colors and wrapping, clears on New task, and passes 39 static plus 19 real-browser checks. No product route invokes it, and network, model-tool, active-navigation, persistence, and follow-up authority remain false.
- [ ] Add explicit selected-page retrieval only after DNS/IP revalidation, redirect controls, textual content allowlisting, byte/time limits, inert extraction, and no page execution pass.
  ...
  - [x] Add a development-only fixed-Wikipedia selected-page transport with fresh query/citation/destination binding, two-pass public DNS validation, pinned TLS, strict JSON/plain-text extraction, no redirects, credentials, cookies, proxy inheritance, page execution, files, or follow-up, 41 hostile checks, matching Windows/Linux source evidence, and residue-free Linux cleanup. Runtime, UI, model-tool, active navigation, persistence, package, hostile-local-server, and macOS authority remain false.
