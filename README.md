...
The Models panel can filter installed models without network access. An explicitly consented public-catalog search sends only the bounded search phrase to Ollama's fixed HTTPS catalog. Uninstalled results may be retained in browser memory as a desired model, but remain candidate-only with unverified evidence, unknown hardware fit, required license review, and execution disabled. Haven 42 can display and copy an engine-constructed `ollama pull <validated-name>` instruction; it never runs that command or calls the Ollama pull API. Reconnect after an external installation so Haven 42 can verify the installed name and digest.
