...
Models and About are dedicated primary views rather than sidebar destinations. The Models view shows installed choices immediately, configures Chat, Writing, and Summarization independently, and keeps catalog search beside selection. Only one primary Chat/Software/Images/Models/About panel is rendered at a time. In text tools, Enter submits, Shift+Enter inserts a new line, and Up/Down recalls older/newer prompts without replacing normal multiline movement. Prompt recall defaults to 20 entries and offers 50 or 100 in System; content stays in browser memory and clears with New task, capability/provider change, or shutdown.
